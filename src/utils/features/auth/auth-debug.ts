...
export function clearAuthCache() {
  ...
  try {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (
        key &&
        (key.startsWith("@@auth0spajs@@") || key.startsWith("auth0.") || key.includes("auth0"))
      ) {
...
if (process.env.NODE_ENV === "development") {
  if (typeof window !== "undefined") {
    ...
    (window as unknown as { clearAuthCache: typeof clearAuthCache }).clearAuthCache =
      clearAuthCache;
    (window as unknown as { debugAuthState: typeof debugAuthState }).debugAuthState =
      debugAuthState;
  }
}
